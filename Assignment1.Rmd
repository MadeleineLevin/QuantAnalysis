...
centered_model <- lm(census2020$num_arrests~ census2020$num_subway + census2020$pop + census2020$num_historic, centered_data)
...
data_scaled <- census2020 %>%
  mutate(pop = pop / 1000, num_wifi = num_wifi/10)

model_scaled <- lm(census2020$num_arrests~ census2020$num_subway + census2020$pop + census2020$num_historic + census2020$num_wifi, data_scaled)

export_summs(full_model3, centered_model, model_scaled,
             error_format = "(p = {p.value})",
             error_pos = "same",
...
The number outside of the parentheses is the coefficient. The p values are only significant for subway, pop, and historic. 
...
Our various transformations did not change the data, but it did make the finding about the wifi spots more significant when they were scaled.
