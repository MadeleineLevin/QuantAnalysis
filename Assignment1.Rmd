...
interaction_model <- lm(census2020$allarrestssummer ~ census2020$num_subway + census2020$pop + census2020$num_parks + census2020$num_historic + census2020$num_wifi + census2020$num_museum + census2020$num_subway*census2020$num_POPs, centered_data)
...
```{r}
interact_plot(interaction_model, pred = census2020$num_subway, modx = census2020$num_POPs,
              interval = TRUE) +
  scale_x_continuous(breaks = breaks <- c(-2, -1, 0, 1, 2),
                     labels = round(breaks * 10 + mean(census2020$num_subway, na.rm = TRUE)),
                     name = "Number of Subways") +
  scale_y_continuous(name = "Size of Population")
```
